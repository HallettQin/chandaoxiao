...
class wxpay
{
    ...
    //支付退款
    function payrefund($order, $payment, $amount, $change_desc) {
        include_once('wxpay_payment/wechat.php');
        // 配置参数
        $wechat = new wechat($payment);
        $arr = [];
        //退货单号
        ...
        if ($main_order_id && !$order['log_id']) {
            $sql = 'SELECT log_id FROM' .$GLOBALS['ecs']->table('pay_log').
                " WHERE order_id = '".$order['order_id']."' AND order_type = '".PAY_ORDER."' ";
            $order['log_id'] = $GLOBALS['db']->getOne($sql);
            $arr['out_refund_no'] = $order['order_id'].'O'.$order['log_id'].'O'.$order['ret_id'];

            $main_order = order_info($main_order_id);
            $log_id = zxk_get_paylog_id($main_order['order_id'], $pay_type = PAY_ORDER);
            $arr['total_fee'] = $main_order['total_fee'] * 100; //订单总金额
        } else {
            $arr['out_refund_no'] = $order['order_id'].'O'.$order['log_id'].'O'.$order['ret_id'];
            $log_id = $order['log_id'];
            $arr['total_fee'] = $order['total_fee'] * 100; //订单总金额
        }
        ;
        ...
        $arr['transaction_id'] = $pay_log['transid'];


        $arr['refund_fee'] = $amount * 100;//退款金额
        $arr['refund_desc'] = $change_desc;//退款原因
        return $wechat->payrefund($arr);
    }
