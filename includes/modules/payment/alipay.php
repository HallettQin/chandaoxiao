...
class alipay
{
    ...
    //退款
    public function payrefund($order, $payment_config, $amount, $change_desc) {
        $arr = [];
        //支付单号
        ...
        if ($main_order_id && !$order['log_id']) {
            ...
            $sql = 'SELECT order_id, order_sn, pay_id, order_amount ' . 'FROM ' . $GLOBALS['ecs']->table('order_info') .
                " WHERE order_id = '$main_order_id'";
            $main_order = $GLOBALS['db']->getRow($sql);
            $log_id = zxk_get_paylog_id($main_order['order_id'], $pay_type = PAY_ORDER);
            $arr['out_trade_no'] = $main_order['order_sn'] . 'O' . $log_id;
        } else {
            $arr['out_trade_no'] = $order['order_sn'] . 'O' . $order['log_id'];
        }

        $arr['refund_fee'] = $amount;
        $arr['refund_no'] = $order['order_sn'].'O'.$order['log_id'].'O'.$order['ret_id'];
        $arr['reason'] = $change_desc;

        //获取支付单号
        try {
            $ret = \Payment\Client\Refund::run(\Payment\Config::ALI_REFUND, $this->getConfig(), $arr);
            if ($ret && 'T' == $ret['is_success']) {
                return true;
            }
        }
        catch (\Payment\Common\PayException $e) {
