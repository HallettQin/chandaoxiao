...
function wholesale_list($ru_id) 
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) 
	{
        ...
        $stat = wholesale_stat($row['act_id']);
...
function lib_get_wholesale_volume_price($goods_id = 0, $goods_number = 0)
{
...
function get_wholesale_goods_info($act_id, $warehouse_id = 0, $area_id = 0, $select = array())     
{
	...
	$sql = ' SELECT g.goods_unit,bd.brand_logo,w.goods_id, w.goods_name, w.goods_type, w.rank_ids, w.price_model, ' . $goods_price . ', w.goods_number, w.moq, ' . ' wc.cat_id, ' . ' g.user_id, g.market_price, g.shop_price, g.cost_price, g.goods_desc, g.goods_img, g.brand_id, g.goods_sn, g.goods_weight FROM ' . $GLOBALS['ecs']->table('wholesale') . ' AS w ' . $left_join . $where;
	...
	if ($row !== false) 
	{
		...
		$arr['moq'] = $row['moq'];
		$arr['volume_price'] = lib_get_wholesale_volume_price($row['goods_id']);
...
function lib_wholesale_calculate_goods_price($goods_id = 0, $goods_number = 0)
{
...
function lib_wholesale_calculate_cart_goods_price($goods_id = 0, $rec_ids = '')
{
	...
	$price_info = lib_wholesale_calculate_goods_price($goods_id, $total_number);
...
//查询批发购买N件商品
function wholesale_get_buy_cart_goods_number($cart_value) {
    ...
    $sql = "SELECT SUM(c.goods_number) FROM " . $GLOBALS['ecs']->table('wholesale_cart') .
        " AS c LEFT JOIN ".$GLOBALS['ecs']->table('goods').
        " AS g ON c.goods_id = g.goods_id WHERE $where " . $c_sess . $goodsIn;
 ...
 *
 **/
function wholesale_order_fee($goods, $consignee) {
    $step = '';
    $shipping_list = array();
 ...
 * @return  array
 */
function wholesale_flow_order_info() {
...
function wholesale_cart_goods($goods_id = 0, $rec_ids = '') 
{
	...
	foreach ($ru_ids as $key => $val ) 
	{
		...
		{
			lib_wholesale_calculate_cart_goods_price($g, $rec_ids);
			...
			foreach ($res as $k => $v ) 
			{
				...
				$res[$k]['goods_attr'] = lib_wholesale_get_goods_attr_array($v['goods_attr_id']);
			...
			{
				$goods_data['volume_price'] = lib_get_wholesale_volume_price($g, $total_number);
			}
			$goods_data['list'] = $res;
...
function lib_wholesale_get_goods_attr_array($goods_attr_id = '')
{
	if (empty($goods_attr_id)) 
	{
		return false;
...
function get_select_record_data($goods_id = 0, $attr_num_array = array()) 
{
	...
	{
		$data = array();
		$data['main_attr'] = lib_wholesale_get_goods_attr_array($key);
		foreach ($val as $k => $v ) 
		{
			$a = array();
			$a['attr_num'] = $v;
			$b = lib_wholesale_get_goods_attr_array($k);
			$c = $b[0];
			$a = array_merge($a, $c);
			$data['end_attr'][] = $a;
		}
