...
//检查采购商权限
if (!is_scscp_admin() && !is_scscp_seller($merchant_id)) {
    priv();
}
...
if($templates_mode == 1 && $cat_id == 0 && !in_array($mode, ['group_buy', 'presale', 'wholesale', 'sample']) && $display == ''&& $brand == 0 && $filter_attr=='' && $keyword == ''&& $price_max=='' && $price_min == ''){
...
if (!$smarty->is_cached($dwt, $cache_id))
{
   assign_template('',array(),$merchant_id);
    ...
    $goodslist = store_get_goods($children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order, $merchant_id, $region_id, $area_info['region_id'], $keyword, $mode);
//    $goodslist = store_goods_list($children, $brand, $price_min, $price_max, $ext, 1000, $page, $sort, $order, $merchant_id, $region_id, $area_info['region_id'], $keyword, $mode);
    ...
    array_multisort($goods_ids,SORT_NUMERIC,$sort_order, $goodslist);
...
$smarty->display($dwt, '', $not);


function store_goods_list($children, $brand, $min, $max, $ext, $size, $page, $sort, $order, $merchant_id, $warehouse_id = 0, $area_id = 0, $keyword, $mode) {
    //ecmoban模板堂 --zhuo start
    $now = gmtime();
    $where = 1;
    ...
    if ($children == '') {
        ...
        $where .= " AND g.user_id = '$merchant_id' "." AND $children ";
    }
    ...
    if ($GLOBALS['_CFG']['review_goods'] == 1) {
        $where .= ' AND b.review_status > 2 ';
    }

    if ($keyword)
    {
        $where .= " AND (g.goods_name LIKE '%$keyword%' OR b.goods_name LIKE '%$keyword%') ";
    }
    ...
    switch($mode) {
        case 'group_buy':
            ...
            $fields .= ',b.start_time AS start_date, b.end_time AS end_date ';
            break;
        case 'presale':
    ...
    $sql = "select * from (SELECT b.*, IFNULL(g.goods_thumb, '') AS goods_thumb,g.shop_price,g.market_price".$fields.
        " FROM " . $table . " AS b " .
        " inner JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON b.goods_id = g.goods_id " .
        $join .
        " WHERE " . $where  . " ORDER BY b.act_id desc  limit 0,10000) as bb GROUP BY bb.goods_id ";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if (in_array($mode, ['group_buy', 'presale', 'sample'])) {
            ...
            $nowprice = $row['price_ladder'][0]['price']; //现价

            $row['volume_price'] = price_format($price_ladder[0]['price']);
        }
        ...
        $row['cur_amount'] = $stat['total_goods'];
        $row['mode'] = $mode;

        //商品图片
        $row['goods_thumb'] = get_image_path($row['goods_id'], $row['goods_thumb'], true);

        $_mode = $mode == 'wholesale' ? 'wholesale_goods' : $mode;
        $row['url'] = build_uri($_mode, ['acid'=>$row['act_id'], 'aid'=>$row['act_id'], 'gbid' => $row['act_id'],'id'=>$row['act_id'], 'presaleid'=>$row['act_id'], 'act'=>'view']);;
 ...
 * @return  array
 */
function store_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order, $merchant_id, $warehouse_id = 0, $area_id = 0, $keyword, $mode) {
    //ecmoban模板堂 --zhuo start
    if ($children == '') {
    ...
    $leftJoin1 = $leftJoin ." inner JOIN " . $GLOBALS['ecs']->table('goods_activity') . " AS pa ON pa.goods_id = g.goods_id ";
    $leftJoin2 = $leftJoin ." inner JOIN " . $GLOBALS['ecs']->table('presale_activity') . " AS pa ON pa.goods_id = g.goods_id ";
    $leftJoin3 = $leftJoin ." inner JOIN " . $GLOBALS['ecs']->table('sample_activity') . " AS pa ON pa.goods_id = g.goods_id ";
    $leftJoin4 = $leftJoin ." inner JOIN " . $GLOBALS['ecs']->table('wholesale') . " AS pa ON pa.goods_id = g.goods_id ";

    $where .= ' AND pa.review_status = 3 ';

    $sql1 = "SELECT pa.act_id,pa.ext_info, 'price_model',g.goods_id,g.goods_name,g.sales_volume,".
        "g.goods_thumb , g.goods_img,g.good_mode,pa.add_time ".
        "FROM " . $GLOBALS['ecs']->table('goods') . " AS g " .
        $leftJoin1 .
        "WHERE $where $ext";
    $sql2 = "SELECT pa.act_id,pa.ext_info,'price_model',g.goods_id,g.goods_name,g.sales_volume,".
        "g.goods_thumb , g.goods_img ,g.good_mode,pa.add_time  ".
        "FROM " . $GLOBALS['ecs']->table('goods') . " AS g " .
        $leftJoin2 .

        "WHERE $where $ext";
    $sql3 = "SELECT pa.act_id,pa.ext_info, 'price_model',g.goods_id,g.goods_name,g.sales_volume,".
        "g.goods_thumb , g.goods_img ,g.good_mode,pa.add_time  ".
        "FROM " . $GLOBALS['ecs']->table('goods') . " AS g " .
        $leftJoin3 .

        "WHERE $where $ext";
    $sql4 = "SELECT pa.act_id,'', pa.price_model ,g.goods_id,g.goods_name,g.sales_volume,".
        "g.goods_thumb , g.goods_img ,g.good_mode,pa.add_time  ".
        "FROM " . $GLOBALS['ecs']->table('goods') . " AS g " .
        $leftJoin4 .
        "WHERE $where $ext ";

    if ($mode && in_array($mode, ['group_buy', 'presale', 'sample', 'wholesale'])) {
        switch($mode) {
            case 'group_buy':
                $sql = "SELECT * FROM (SELECT * FROM ( " .$sql1 .") AS b order by act_id desc limit 0, 10000) AS c GROUP BY goods_id ORDER BY $sort $order";
                break;
            case 'presale':
                $sql = "SELECT * FROM (SELECT * FROM ( " .$sql2 .") AS b order by act_id desc limit 0, 10000) AS c GROUP BY goods_id ORDER BY $sort $order";
                break;
            case 'sample':
                $sql = "SELECT * FROM (SELECT * FROM (" .$sql3 .") AS b order by act_id desc limit 0, 10000) AS c GROUP BY goods_id ORDER BY $sort $order";
                break;
            case 'wholesale':
                $sql = "SELECT * FROM (SELECT * FROM ( " .$sql4 .") AS b order by act_id desc limit 0, 10000) AS c GROUP BY goods_id ORDER BY $sort $order";
                break;
        }
    } else {
        $sql = "SELECT * FROM (SELECT * FROM ( ".$sql1 .' UNION ALL '.$sql2 . ' UNION ALL '.$sql3  .' UNION ALL '.$sql4 .") AS b order by act_id desc limit 0, 10000) AS c GROUP BY goods_id ORDER BY $sort $order";

    }

    /* 获得商品列表 */
//    $sql = "SELECT * FROM (SELECT * FROM (".
//            "SELECT pa.act_id,pa.ext_info, 'price_model',g.goods_id,g.goods_name,g.sales_volume,".
//            "g.goods_thumb , g.goods_img,g.good_mode,pa.add_time ".
//            "FROM " . $GLOBALS['ecs']->table('goods') . " AS g " .
//            $leftJoin1 .
//            "WHERE $where $ext".
...
//            "WHERE $where $ext".
//            "UNION ALL ".
//            "SELECT pa.act_id,'', pa.price_model ,g.goods_id,g.goods_name,g.sales_volume,".
//            "g.goods_thumb , g.goods_img ,g.good_mode,pa.add_time  ".
//            "FROM " . $GLOBALS['ecs']->table('goods') . " AS g " .
//            $leftJoin4 .
//            "WHERE $where $ext ".
//            ") AS b order by act_id desc limit 0, 10000) AS c GROUP BY goods_id ORDER BY $sort $order";
    $res = $GLOBALS['db']->selectLimit($sql, $size, ($page - 1) * $size);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        //冲定义价格
        if (in_array($row['good_mode'], [1, 2, 3])) {
            $ext_info = unserialize($row['ext_info']);
            $row = array_merge($row, $ext_info);
        ...
        if (1 == $row['good_mode']) {
            //拼单
            $row['url'] = build_uri('group_buy', ['act'=>'view', 'gbid'=>$row['act_id']]);
        } else if (2 == $row['good_mode']) {
            //预定
            $row['url'] = build_uri('presale', ['act'=>'view', 'presaleid'=>$row['act_id']]);
        } else if (3 == $row['good_mode']) {
            //样品
            $row['url'] = build_uri('sample', ['act'=>'view', 'acid'=>$row['act_id']]);
        } else if (4 == $row['good_mode']) {
            //批发
            $row['url'] = build_uri('wholesale_goods', ['aid'=>$row['act_id']]);
        }
