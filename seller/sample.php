...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('menu_select', array('action' => '03_promotion', 'current' => '20_sample'));
    $smarty->assign('full_page', 1);
    $smarty->assign('ur_here', $_LANG['sample_list']);
    $smarty->assign('action_link', array('href' => 'sample.php?act=add', 'text' => $_LANG['add_sample'], 'class' => 'icon-plus'));

    $list = sample_list($adminru['ru_id']);
    $page_count_arr = seller_page($list, $_REQUEST['page']);
    $smarty->assign('page_count_arr', $page_count_arr);
    $smarty->assign('sample_list', $list['item']);
    ...
    make_json_result($smarty->fetch('sample_list.dwt'), '', array('filter' => $list['filter'], 'page_count' => $list['page_count']));
    ...
    if (isset($_POST['checkboxes'])) {
        ...
        foreach ($_POST['checkboxes'] as $key => $id) {
            ...
            admin_log(addslashes($presale['goods_name']) . '[' . $id . ']', 'remove', 'sample');
            $del_count++;
        }
    ...
    if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
        ...
        $adminru = get_admin_ru_id();
        $sample = array('act_name' => $act_name, 'act_desc' => $_POST['act_desc'], 'goods_id' => $goods_id, 'user_id' => $adminru['ru_id'], 'goods_name' => $goods_name, 'ext_info' => serialize(array('price_ladder' => $price_ladder)), 'production_cycle'=>$pruduct_cycle);
        clear_cache_files();

        if (0 < $sample_id) {
            $sample['review_status'] = 1;
        ...
        $id = intval($_GET['id']);
        $sample = sample_info($id, 0, 'seller');

        if ($sample['user_id'] != $adminru['ru_id']) {
            $url = 'sample.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
            ecs_header('Location: ' . $url . "\n");
            exit();
        }

        if (0 < $sample['valid_order']) {
            make_json_error($_LANG['error_exist_order']);
        }

        $sql = 'DELETE FROM ' . $ecs->table('sample_activity') . ' WHERE act_id = \'' . $id . '\' LIMIT 1';
        $db->query($sql);
        //日志
