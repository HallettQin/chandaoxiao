...
function order_list($page = 0)
{
    ...
    if ($result === false)
    {
        ...
        $filter['wholesale_id'] = isset($_REQUEST['wholesale_id']) ? intval($_REQUEST['wholesale_id']) : 0; // 预售id
        ...
        //如果没有 添加24小时后订单可见
        $filter['end_time'] = empty($_REQUEST['end_time']) ? gmtime() - 24 * 60 * 60 : (strpos($_REQUEST['end_time'], '-') > 0 ?  local_strtotime($_REQUEST['end_time']) : $_REQUEST['end_time']);
        ...
        //已取消订单查看不了 by zxk
        $where .= ' AND o.order_status not in('.OS_CANCELED.','.OS_INVALID.')';
        ...
        $sql = "SELECT ifnull(bai.is_stages,0) is_stages, o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.pay_status, o.order_amount, o.money_paid, o.is_delete," .
                ...
                "o.shipping_time, o.confirm_take_time, o.auto_delivery_time, o.consignee, o.address, o.email, o.tel, o.mobile, o.extension_code AS o_extension_code, " .
...
function download_orderlist($result) {
    ...
    $data = i('订单号,商品名称,商品类目,货号,规格,退货规格,商家名称,下单会员,下单时间,确认收货时间,退单金额,总金额,配送费用,保价费用,支付费用,余额金额,积分金额,红包金额,发票税额,折扣金额,优惠券金额,应付金额,确认状态,付款状态,发货状态,订单来源,支付方式'."\n");
    $count = count($result);
    for ($i = 0; $i < $count; $i++) {
        ...
        $data .= $order_sn . ',' . $goods_name  . ','. $cat_name  . ','. $goods_sn  . ','. $guige  . ','. $return_guige  . ','. $seller_name  . ',' . $order_user . ',' .
                $order_time . ',' .$confirm_take_time . ','.$return_amount . ','. i($result[$i]['total_fee']) . ',' .
...
function timing_cancel_order() {
    ...
    $where .= " AND (SELECT og.ru_id FROM " . $GLOBALS['ecs']->table('order_goods') .' as og' . " WHERE og.order_id = oi.order_id LIMIT 1) = '$merchant_id' ";
