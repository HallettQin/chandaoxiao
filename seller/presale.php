...
if ($_REQUEST['act'] == 'list') {
	...
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
		...
		if (isset($_POST['finish'])) {
			...
			$adminru = get_admin_ru_id();
			$presale = array('act_name' => $act_name, 'act_desc' => $_POST['act_desc'], 'cat_id' => intval($_POST['cat_id']), 'goods_id' => $goods_id, 'user_id' => $adminru['ru_id'], 'goods_name' => $goods_name, 'start_time' => $start_time, 'end_time' => $end_time, 'pay_start_time' => $pay_start_time, 'pay_end_time' => $pay_end_time, 'deposit' => $deposit,  'restrict_amount' => $restrict_amount, 'ext_info' => serialize(array('price_ladder' => $price_ladder)), 'production_cycle'=> $production_cycle, 'moq' => $moq);
            clear_cache_files();
