...
function bill_detail_download_list($result) {
    if (empty($result)) {
        return i('没有符合您要求的数据！^_^');
    }
    ...
    $data = i("编号\t订单号\t商品名称\t货号\t规格\t退货规格\t快递名称\t快递单号\t订单金额\t有效分佣金额\t运费金额\t退单金额\t分销金额\t收取比例\t收取佣金\t应结比例\t本期应结\t本期应结\t订单状态" . "\n");
    ...
    foreach ($result as $k => $v) {
        ...
        $row = [];
        $row[id] = '#'.$v[id];
        $row['order_sn'] = '#'.$v[order_sn];
        $row['goods_name'] = $goods_list[0][goods_name];

        //货号
        $goods_sn = $goods_list[0]['goods_sn'];
        ...
        $return_guige = '';
        if ($v['return_rec_id']) {
            $ret_id = $v['return_rec_id'];
            $return_list = get_return_goods($ret_id);


            $return_guige .= "退货：";
            foreach ($return_list as $r) {
                $goods_attr = get_table_date('order_return', 'ret_id = '.$ret_id, array('goods_attr'), 2);
                $return_guige .= str_replace(array("\t", "\r\n", "\r", "\n"), ' ', $goods_attr).' '.$r['return_number'] ."件    ";
            }
        }  else {
            $return_guige = '无';
        }
        ...
        //规格
        $row['guige'] = $guige;
        $row['return_guige'] = $return_guige;

        //快递单号
        $order = order_info($order_id);
        //快递名称
        $row[shipping_name] = $order['shipping_name'];

        $row[invoice_no] = $order['invoice_no'];

        $row[format_order_amount] = $v[format_order_amount] . '(含运费)';
        $row[format_commission_total_fee] = $v[format_commission_total_fee] . '(不含运费)';
        $row[format_shipping_fee] = $v[format_shipping_fee];
        $row[format_return_amount] = $v[format_return_amount];
        $row[format_drp_money] = $is_dir ? $v[format_drp_money] : "";

        $row[gain_proportion] = $v[gain_proportion] ."%";
        $row[format_gain_commission] = $v[format_gain_commission] . "(不含运费)";

        $row[should_proportion] = '【订单】：'. $v[should_proportion] ."%";
        $row[format_should_amount] = $v[format_should_amount] .'(含运费)';
        $row[format_shipping_should_amount] = $v[format_shipping_should_amount] .'(扣除运费)';

        $row[status] = $GLOBALS['_LANG']['os'][$v['order_status']] ." " .$GLOBALS['_LANG']['ps'][$v['pay_status']] ." " .$GLOBALS['_LANG']['ss'][$v['shipping_status']];
...
//账单导出
function merchants_commission_download_list($result) {
    if (empty($result)) {
        return i('没有符合您要求的数据！^_^');
    }
    ...
    $data = i("账单编号\t订单金额\t运费金额\t退单金额\t分销金额\t收取比例\t收取佣金\t应结比例\t本期应结\t本期应结\t冻结资金\t未收货订单(个)\t出账状态\t出账时间" . "\n");
    $count = count($result);
    for ($i = 0; $i < $count; $i++) {
        $row = [];
        $row[bill_sn] = '#'.$result[$i][bill_sn];
        $row[format_order_amount] = $result[$i][format_order_amount].(含运费);

        $row[format_shipping_fee] = $result[$i][format_shipping_fee];
        $row[format_return_amount] = $result[$i][format_return_amount];
        $row[format_drp_money] = $is_dir ? $result[$i][format_drp_money] :"";
        $row[gain_proportion] = $result[$i][gain_proportion];

        $row[format_gain_commission] = $result[$i][format_gain_commission].'(不含运费)';
        $row[should_proportion] = $result[$i][should_proportion];//*********
        $row[format_should_amount] = $result[$i][format_should_amount]; //*********
        $row[format_shipping_should_amount] = $result[$i][format_shipping_should_amount].'(扣除运费)'; //*********
        $row[format_frozen_money] = $result[$i][format_frozen_money];
        $row[notake_order_count] = $result[$i][notake_order_count];
        $row[chargeoff_status] = '已出账';
        $row['format_confirmconfirm_take_time'] = $result[$i][format_confirmconfirm_take_time] ? $result[$i][format_confirmconfirm_take_time] : '未出账';
...
if ($_REQUEST['act'] == 'list') {
    ...
    $tab_menu[] = array('curr' => 0, 'text' => $_LANG['brokerage_order_list'], 'href' => 'merchants_commission.php?act=order_list&id=' . $adminru['ru_id']);
    ...
    $start_date = date('Ymd', local_strtotime($_REQUEST['start_date']));
    $end_date = date('Ymd', local_strtotime($_REQUEST['end_date']));
    $filename = '账单导出'.$start_date. '_' . $end_date;
    ...
    $result = bill_detail_list2();
    $data = '';
    $data .= i('账务明细查询')."\n";
    $data .= i('起始时间：'.date('Ymd H:i:s', local_strtotime($_REQUEST['start_date'])))."\n";
    $data .= i('结束时间：'.date('Ymd H:i:s', local_strtotime($_REQUEST['end_date'])))."\n";
    $data .= i('账务明细列表')."\n";

    $bill_list = bill_detail_download_list($result['bill_list']);
    $data .= $bill_list;
    echo $data;
}
...
if ($_REQUEST['act'] == 'merchant_download') {
    ...
    if (!isset($_REQUEST['start_time'])) {
        $start_time = local_strtotime("-1 month");
        $start_date = local_date('Ymd', $start_time);
    }

    if (!isset($_REQUEST['end_time'])) {
        $end_time = local_strtotime('+1 day');
        $end_date = local_date('Ymd', $end_time);
    }

    $_REQUEST['start_time'] = local_date('Y-m-d', $start_time);
    $_REQUEST['end_time'] = local_date('Y-m-d', $end_time);
    ...
    $filename = '账单导出'.$start_date. '_' . $end_date;
    $smarty->assign('action_link',  array('text' => "导出账单", 'href' => 'merchants_commission.php?act=download&start_date=' . $start_date . '&end_date=' . $end_date ));
    $smarty->display('merchants_bill_detail2.dwt');
}
    ...
    $data = '';
    $data .= i('账务明细查询')."\n";
    $data .= i('起始时间：'.$bill['format_start_time'])."\n";
    $data .= i('结束时间：'.$bill['format_end_time'])."\n";
    $data .= i('账务明细列表')."\n";
	...
	$sort_flag = sort_flag($result['filter']);
	$smarty->assign($sort_flag['tag'], $sort_flag['img']);
	make_json_result($smarty->fetch('merchants_bill_detail2.dwt'), '', array('filter' => $result['filter'], 'page_count' => $result['page_count']));
}
