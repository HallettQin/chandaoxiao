 ...
 * @param   object  $filters    过滤条件
 */
function get_goods_list_instock($filter){

    $filter->keyword = json_str_iconv($filter->keyword);
    $where = get_where_sql($filter); // 取得过滤条件
    $join = ' inner join '.$GLOBALS['ecs']->table('wholesale'). 'AS a ON a.goods_id = g.goods_id ';

    $where .= ' AND a.review_status = 3';

    /* 取得数据 */
    $sql = 'SELECT a.act_id, g.goods_id, g.goods_name '.
        'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $join . $where .
        ' LIMIT 50';

    $row = $GLOBALS['db']->getAll($sql);

    array_unshift($row, ['act_id'=>0, 'goods_id'=>0, 'goods_name'=>'请选择']);
...
//    $where .= isset($filter->presale) ? " AND is_on_sale = 0 " : '';
    ...
    //商品模式
    $where .= isset($filter->good_mode) ? " AND good_mode = $filter->good_mode " : '';
