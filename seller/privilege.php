...
elseif ($_REQUEST['act'] == 'modif')
{
...
} elseif ($_REQUEST['act'] == 'kouling') {
    $smarty->assign('primary_cat',  '口令密码');
    $smarty->assign('ur_here',     '设置');
    ...
    $sql  = "SELECT user_name, mobile_phone ".
        "FROM " .$GLOBALS['ecs']->table('users') . " WHERE user_id = '$ru_id'";

    $user_info = $db->getRow($sql);
    $smarty->assign('phone', $user_info['mobile_phone']);

    //验证码
    $sms_security_code = $_SESSION['sms_security_code'] = rand(1000, 9999);
    $smarty->assign('sms_security_code', $sms_security_code);

    $smarty->assign('form_act', 'kouling_update');

    /* 显示页面 */
    assign_query_info();
    $smarty->display('privilege_kouling.dwt');
} elseif ('kouling_update' == $_REQUEST['act']) {
    ...
    $user_info = $db->getRow($sql);
    if ($user_info['mobile_phone'] != $_SESSION['sms_mobile']) {
        sys_msg($_LANG['mobile_code_error']);
        exit();
    }


    if (!$other['mobile_code'] || $_SESSION['sms_mobile_code'] != $other['mobile_code']) {
        sys_msg($_LANG['mobile_code_error']);
        exit();
    }
