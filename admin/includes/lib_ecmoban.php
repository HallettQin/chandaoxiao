...
function get_shop_name($ru_id = 0, $type = 0)
{
	...
	if ($type == 3) {
		...
		$shop_information['rz_shopName'] = str_replace(array('生产制造商', '品牌供应商', '设计开发公司'), '', $shop_information['rz_shopName']);
	}
...
function presale_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
	...
	if ($res === NULL) {
		...
		if ($data === false) {
			...
			$res = $GLOBALS['db']->getAll($sql);
			$sql = 'SELECT pa.cat_id, COUNT(*) AS goods_num ' . 'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('presale_activity') . ' AS pa ON pa.goods_id = g.goods_id ' . ' WHERE is_delete = 0  ' . ' GROUP BY pa.cat_id';
			$res2 = $GLOBALS['db']->getAll($sql);
			$sql = 'SELECT pc.cat_id, COUNT(*) AS goods_num FROM ' . $GLOBALS['ecs']->table('presale_cat') . ' AS pc ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('presale_activity') . ' AS pa ON pc.cat_id = pa.cat_id ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = pa.goods_id ' . 'WHERE g.is_delete = 0  GROUP BY pa.cat_id';
...
function presale_stat($presale_id, $deposit, $user_id = 0)
{
	...
	$sql = 'SELECT COUNT(*) AS total_order, SUM(g.goods_number) AS total_goods ' . 'FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o, ' . $GLOBALS['ecs']->table('order_goods') . ' AS g ' . ' WHERE o.order_id = g.order_id ' . 'AND o.extension_code = \'presale\' ' . 'AND o.extension_id = \'' . $presale_id . '\' ' . 'AND g.goods_id = \'' . $goods_id . '\' ' . 'AND (order_status = \'' . OS_CONFIRMED . '\')';
	$stat = $GLOBALS['db']->getRow($sql);

    //不设置尾款直接支付
    if (!$deposit) {
        $sql .= " AND pay_status = " . PS_PAYED;
    }
