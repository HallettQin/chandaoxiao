...
function commission_bill_list($ajax_bill = 0)
{
	...
	if ($result === false) {
		...
		if ($ajax_bill == 0) {
			$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('seller_commission_bill') . ' AS b ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_server') . ' AS s ON b.seller_id = s.user_id AND b.bill_cycle = s.cycle ' . 'WHERE ' . $where . ' AND b.bill_cycle = s.cycle ';
			$filter['record_count'] = $GLOBALS['db']->getOne($sql);
			$filter['page_count'] = 0 < $filter['record_count'] ? ceil($filter['record_count'] / $filter['page_size']) : 1;
