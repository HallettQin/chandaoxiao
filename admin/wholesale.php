...
if ($_REQUEST['act'] == 'list')
{
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('wholesale'), $other, "UPDATE", "act_id in (".$ids.")");

    $result['type'] = $type;
    admin_log([$ids], 'audit', 'wholesale');
    ...
    $goods_list_type = get_wholesale_type_number($adminru['ru_id']);
    ...
    //审核页面
    $smarty->display('wholesale_review_list.dwt');
    exit;
}
else if ($_REQUEST['act'] == 'query')
{
	...
	$sort_flag = sort_flag($list['filter']);
	$smarty->assign($sort_flag['tag'], $sort_flag['img']);
    $tpl = $list['filter']['review_status'] ? 'wholesale_review_list.dwt' : 'wholesale_list.dwt';
...
function get_wholesale_type_number($ru_id) {
    $result = get_filter();
    $where = '';
    if ($result === false) {
    ...
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('wholesale') . ' AS ga ' . ' WHERE 1 ' . $where;
