...
if ($_REQUEST['act'] == 'list') {
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('presale_activity'), $other, "UPDATE", "act_id in (".$ids.")");

    $result['type'] = $type;
    admin_log([$ids], 'audit', 'presale');
    ...
    $goods_list_type = get_presale_type_number($adminru['ru_id']);
    ...
    //审核页面
    $smarty->display('presale_review_list.dwt');
    exit;
}
    ...
    $tpl = $list['filter']['review_status'] ? 'presale_review_list.dwt' : 'presale_list.dwt';
...
//						$order['goods_amount'] = $goods_amount;
            ...
            //生产周期
            $production_cycle = intval($_POST['production_cycle']);
			...
			$presale = array('act_name' => $act_name, 'act_desc' => $_POST['act_desc'], 'cat_id' => intval($_POST['cat_id']), 'goods_id' => $goods_id, 'goods_name' => $goods_name, 'start_time' => $start_time, 'end_time' => $end_time, 'pay_start_time' => $pay_start_time, 'pay_end_time' => $pay_end_time, 'deposit' => $deposit,  'restrict_amount' => $restrict_amount, 'production_cycle'=> $production_cycle, 'ext_info' => serialize(array('price_ladder' => $price_ladder)),'moq' => $moq);
			clear_cache_files();
...
function get_presale_type_number($ru_id) {
    $result = get_filter();
    $where = '';
    if ($result === false) {
        $filter['keyword'] = empty($_REQUEST['keyword']) ? '' : trim($_REQUEST['keyword']);
