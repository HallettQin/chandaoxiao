...
if ($_REQUEST['act'] == 'list') {
    ...
    if (!empty($pid)) {
        $sql = " select position_id, position_name, position_model from ".$GLOBALS['ecs']->table('touch_ad_position')." where position_id='".$pid."' limit 1";
        $position_info = $GLOBALS['db']->getRow($sql);
        if(!empty($position_info['position_model'])) {
            //初始广告位模型($ad_model)和模型结构($ad_model_structure)
            $smarty->assign('position', json_encode($position_info));
        }
    }
	...
	$sql = 'INSERT INTO ' . $ecs->table('touch_ad') . " (position_id,media_type,ad_name,is_new,is_hot,is_best,public_ruid,ad_link,ad_code,start_time,end_time,link_man,link_email,link_phone,click_count,enabled, link_color, ad_type, goods_name, b_title, s_title)\r\n    VALUES ('" . $_POST['position_id'] . "',\r\n            '" . $_POST['media_type'] . "',\r\n            '" . $ad_name . "',\r\n            '" . $is_new . "',\r\n            '" . $is_hot . "',\r\n            '" . $is_best . "',\r\n            '" . $public_ruid . "',\r\n            '" . $ad_link . "',\r\n            '" . $ad_code . "',\r\n            '" . $start_time . "',\r\n            '" . $end_time . "',\r\n            '" . $_POST['link_man'] . "',\r\n            '" . $_POST['link_email'] . "',\r\n            '" . $_POST['link_phone'] . "',\r\n            '0',\r\n            '1',\r\n            '" . $link_color . "',\r\n            '" . $ad_type . "',\r\n            '" . $goods_name. "',\r\n            '" . $b_title. "',\r\n            '" . $s_title . '\')';
	$db->query($sql);
	...
	$link[1]['href'] = 'touch_ads.php?act=list'. '&pid=' . $_POST['position_id'];
	$link[2]['text'] = $_LANG['continue_add_ad'];
	$link[2]['href'] = 'touch_ads.php?act=add'. '&pid=' . $_POST['position_id'];
	...
	$sql = 'UPDATE ' . $ecs->table('touch_ad') . ' SET ' . 'position_id = \'' . $_POST['position_id'] . '\', ' . 'ad_name     = \'' . $_POST['ad_name'] . '\', ' . 'ad_link     = \'' . $ad_link . '\', ' . 'link_color  = \'' . $link_color . '\', ' . 'is_new     = \'' . $is_new . '\', ' . 'is_hot     = \'' . $is_hot . '\', ' . 'is_best     = \'' . $is_best . '\', ' . $ad_code . 'start_time  = \'' . $start_time . '\', ' . 'end_time    = \'' . $end_time . '\', ' . 'link_man    = \'' . $_POST['link_man'] . '\', ' . 'link_email  = \'' . $_POST['link_email'] . '\', ' . 'link_phone  = \'' . $_POST['link_phone'] . '\', ' . 'enabled     = \'' . $_POST['enabled'] . '\', ' . 'ad_type  = \'' . $ad_type . '\', ' . 'goods_name  = \'' . $goods_name . '\', ' . 'b_title  = \'' . $b_title . '\', ' . 's_title  = \'' . $s_title . '\' ' . 'WHERE ad_id = \'' . $id . '\'';
	$db->query($sql);
	admin_log($_POST['ad_name'], 'edit', 'ads');
	clear_cache_files();
    $_href = 'touch_ads.php?act=list';

    if ($_POST['position_id']) {
        $_href .= '&pid='.$_POST['position_id'];
    }
	$href[] = array('text' => $_LANG['back_ads_list'], 'href' => $_href);
