...
if ($_REQUEST['act'] == 'commission_download') {
    ...
    if ($_REQUEST['start_time']) {
        $start_date = date('Ymd', local_strtotime($_REQUEST['start_time']));
    }
    if ($_REQUEST['end_Time']) {
        $end_date = date('Ymd', local_strtotime($_REQUEST['end_Time']));
    }
    ...
    $commission_list = commission_bill_list();
    echo merchants_commission_download_list($commission_list['bill_list']);
    exit();
...
if ($_REQUEST['act'] == 'merchant_download') {
    ...
    $smarty->assign('action_link',  array('text' => "导出账单", 'href' => 'merchants_commission.php?act=download&id='.$user_id));
...
function bill_detail_download_list($result) {
    ...
    $data = i("编号\t订单号\t商品名称\t货号\t规格\t退货规格\t快递名称\t快递单号\t订单金额\t有效分佣金额\t运费金额\t退单金额\t分销金额\t收取比例\t收取佣金\t应结比例\t本期应结\t订单状态" . "\n");
...
function merchants_commission_download_list($result) {
    ...
    $data = i("账单编号\t订单金额\t运费金额\t退单金额\t分销金额\t收取比例\t收取佣金\t应结比例\t本期应结\t冻结资金\t未收货订单(个)\t出账状态\t申请状态\t出账时间\t开始时间\t结束时间" . "\n");
    $count = count($result);
    for ($i = 0; $i < $count; $i++) {
        $row = [];
        ...
        $row[format_shipping_amount] = $result[$i][format_shipping_amount];
        ...
        $row[chargeoff_status] = $result[$i]['chargeoff_status'] == 1 ? '已出账' : $result[$i]['chargeoff_status'] == 2 ? '账单结束'.$result[$i][format_settleaccounts_time] : $result[$i]['chargeoff_status'] == 3 ? '关闭账单':  '未出账';
        $row[bill_apply] = $result[$i][bill_apply] == 1 ? '已申请' : '未申请';
        $row['format_chargeoff_time'] = $result[$i][format_chargeoff_time] ? $result[$i][format_chargeoff_time] : '未出账';
        $row['format_start_time'] = $result[$i][format_start_time];
        $row['format_end_time'] = $result[$i][format_end_time];
