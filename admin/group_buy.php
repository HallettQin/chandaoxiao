...
if ($_REQUEST['act'] == 'list') {
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods_activity'), $other, "UPDATE", "act_id in (".$ids.")");

    $result['type'] = $type;

    admin_log([$ids], 'audit', 'group_buy');
    ...
    $goods_list_type = get_group_buy_type_number($adminru['ru_id']);
    ...
    //审核页面
    $smarty->display('group_buy_review_list.dwt');
    exit;
}
	...
	$sort_flag = sort_flag($list['filter']);
	$smarty->assign($sort_flag['tag'], $sort_flag['img']);
    $tpl = $list['filter']['review_status'] ? 'group_buy_review_list.dwt' : 'group_buy_list.dwt';
...
function get_group_buy_type_number($ru_id) {
    $result = get_filter();
    $where = '';
    if ($result === false) {
        $filter['keyword'] = empty($_REQUEST['keyword']) ? '' : trim($_REQUEST['keyword']);
    ...
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('goods_activity') . ' AS ga ' . ' WHERE 1 ' . $where;
