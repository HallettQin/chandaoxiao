 ...
 */
function get_user_orders($user_id, $num = 10, $page = 1, $status = 0, $mode='')
{
    $where = "";
    ...
    if ($status == 1) {
        ...
        //待收货
        $where .= " AND oi.pay_status = " . PS_PAYED . " AND oi.order_status in (" . OS_CONFIRMED . ", " . OS_SPLITED . ", " . OS_SPLITING_PART . ") AND (oi.shipping_status in (" . SS_SHIPPED .  ',' . OS_SHIPPED_PART. "))";
//        $where .= " AND oi.pay_status = " . PS_PAYED . " AND oi.order_status in (" . OS_CONFIRMED . ", " . OS_SPLITED . ", " . OS_SPLITING_PART . ") AND (oi.shipping_status >= " . SS_UNSHIPPED . " AND oi.shipping_status <> " . SS_RECEIVED . ")";
        ...
        //待发货
        $where .= " AND oi.pay_status = " . PS_PAYED . " AND oi.order_status in (" . OS_CONFIRMED . ", " . OS_SPLITED . ", " . OS_SPLITING_PART . ") AND (oi.shipping_status in (" . SS_UNSHIPPED .  ',' . SS_PREPARING.  ',' . SS_SHIPPED_ING ."))";
        $cache_info = S('message_' . $_SESSION['user_id']);
    } elseif ($status == 4) {
        //已完成
    ...
    if ($mode && in_array($mode, ['group_buy', 'presale', 'sample', 'wholesale'])) {
        $where .= " and  oi.extension_code = '".$mode ."'";
    }
    ...
    if (is_dir(APP_TEAM_PATH)) {
        $sql = "SELECT og.ru_id, oi.extension_code,oi.extension_id, oi.main_order_id, oi.consignee,oi.pay_name, oi.order_id, oi.order_sn,oi.pay_time,oi.order_status, oi.shipping_status, oi.pay_status, oi.add_time, oi.shipping_time, oi.auto_delivery_time, oi.sign_time,oi.team_id, " .
            $select .
        ...
        $sql = "SELECT og.ru_id, oi.extension_code,oi.extension_id, oi.main_order_id, oi.consignee,oi.pay_name, oi.order_id, oi.order_sn,oi.pay_time,oi.order_status, oi.shipping_status, oi.pay_status, oi.add_time, oi.shipping_time, oi.auto_delivery_time, oi.sign_time, " .
            $select .
            "(oi.goods_amount + oi.shipping_fee + oi.insure_fee + oi.pay_fee + oi.pack_fee + oi.card_fee + oi.tax - oi.discount) AS total_fee, og.goods_id, " .
            "oi.invoice_no, oi.shipping_name, oi.tel, oi.email, oi.address, oi.province, oi.city, oi.district " .
            " FROM " . $GLOBALS['ecs']->table('order_info') . " as oi" .
    ...
    foreach ($res as $key => $row) {
        ...
        //原始订单状态保存
        $_order_status = $row['order_status'];
        ...
        if ($sign_time > 0) {
            ...
            if ($row['order_status'] != OS_CANCELED && $row['pay_status'] == PS_PAYED && $row['shipping_status'] == SS_SHIPPED) {
                if ($day < $sign_time) {
            ...
            //退款管理
            if (($noTime - $row['pay_time']) / 3600 / 24 < 1 && !in_array($row['shipping_status'], [SS_SHIPPED, SS_RECEIVED, SS_SHIPPED_PART])) {
                $row['handler_return'] = url('user/refound/index', ['order_id' => $row['order_id']]);
            }
        }
        //订单删除
        ...
        $pregress = 0;
        //团购预定样品进度条 by zxk
        if (in_array($row['extension_code'], ['group_buy', 'presale'])) {
            if ('group_buy' == $row['extension_code']) {
                $activity = get_table_date('goods_activity', 'act_id = '.$row['extension_id'], array('act_id', 'is_finished', 'production_cycle', 'end_time'), 0);
            } elseif ('presale' == $row['extension_code']) {
                $activity = get_table_date('presale_activity', 'act_id = '.$row['extension_id'], array('act_id', 'is_finished', 'production_cycle', 'end_time'), 0);
            }

            if ($activity && $activity['is_finished'] == GBS_SUCCEED) {
                //成功
                $confirm_time = $activity['end_time'];
                $production_cycle = $activity['production_cycle'];
                $pregress = (($noTime - $confirm_time) * 100 / ($production_cycle * 24 * 3600)) ;
                $pregress = round($pregress, 1) + 0.1;
                $pregress = $pregress > 100 ? 100 : $pregress;
            }
        }
        if ($row['extension_code'] == 'sample') {
            $activity = get_table_date('sample_activity', 'act_id = '.$row['extension_id'], array('act_id','production_cycle'), 0);
            //待收货 显示
            if ($activity && $activity['production_cycle'] && in_array($row['pay_status'], [PS_PAYED]) && in_array($_order_status, [OS_CONFIRMED, OS_SPLITED, OS_SPLITING_PART]) && in_array($row['shipping_status'], [SS_UNSHIPPED, SS_PREPARING, SS_SHIPPED_ING])) {
                $confirm_time = $row['pay_time'];
                $production_cycle = $activity['production_cycle'];
                $pregress = (($noTime - $confirm_time) * 100 / ($production_cycle * 24 * 3600)) ;
                $pregress = round($pregress, 1) + 0.1;
        ...
        if ($payment_info === false) {
            ...
            if (substr($payment_info['pay_code'], 0, 4) == 'pay_') {
                ...
                if (file_exists(ADDONS_PATH . 'payment/' . $payment_info['pay_code'] . '.php')) {
                    ...
                    if (OS_RETURNED != $order['order_status']) {
                        // 取消 退货订单bug
