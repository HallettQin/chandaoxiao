...
function cart_goods($type = CART_GENERAL_GOODS, $cart_value = '', $ru_type = 0, $warehouse_id = 0, $area_id = 0, $consignee = '', $store_id = 0)
{
    ...
    $sql = "SELECT c.warehouse_id, c.area_id, c.rec_id, c.user_id, c.goods_id, g.user_id as ru_id, g.cat_id, c.goods_name, g.goods_thumb, c.goods_sn, c.goods_number, g.default_shipping, g.goods_weight as goodsweight, " . //储值卡指定分类 liu
        ...
        "c.parent_id, c.group_id, g.is_shipping, g.freight, g.tid, g.shipping_fee, c.store_id " .
        "FROM " . $GLOBALS['ecs']->table('cart') . " AS c " .
        "LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON c.goods_id = g.goods_id " . $goods_where .
        "WHERE $where " . $c_sess .
        "AND rec_type = '$type'" . $goodsIn . " AND c.is_checked = 1 AND c.extension_code <> 'package_buy' GROUP BY c.rec_id order by c.rec_id DESC";
    ...
    foreach ($arr as $k => $v) {
        ...
        if ($extension_code == 'presale') {
            //寻找下一个
            $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('presale_activity') . " WHERE goods_id = ".$goods_id . " AND start_time < " . $now . " AND end_time > " . $now . " AND review_status = 3 ORDER BY act_id DESC";
            $presale = $GLOBALS['db']->getRow($sql);
            if (!$presale) {
                show_message("预售活动已结束");
            }
...
function get_cart_ru_goods_list($goods_list, $cart_value = '', $consignee = '', $store_id = 0)
{
    ...
    foreach ($goods_list as $key => $row) {
        ...
        $arr[$key]['url'] = build_uri('merchants_store', ['urid' => $ru_id], $ru_name);
...
* 获取当前用户订单可使用储值卡列表
* @param user_id 用户ID
* @param cart_category 进货单商品所属分类ID
* @param cart_goods 进货单商品ID
* @return
*/
