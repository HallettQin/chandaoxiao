...
function get_rank_info()
{
    ...
    if (!empty($_SESSION['user_rank'])) {
        ...
        if ($row['special_rank']) {
            ...
            $user_rank = $row;
            return ['next_rank_name' => $next_rank_name, 'next_rank' => $next_rank, 'user_rank' => $user_rank];
        }
    } else {
        return [];
    }
    ...
    if ($res === null) {
        ...
        if ($data === false) {
            ...
            $sql = "SELECT pa.cid, COUNT(*) AS goods_num " .
                ...
                " WHERE is_delete = 0 AND pa.review_status = 3 " .
                " GROUP BY pa.cid";
            ...
            $sql = "SELECT pc.cid, COUNT(*) AS goods_num FROM " . $GLOBALS['ecs']->table('presale_cat') . " AS pc " .
                ...
                "WHERE g.is_delete = 0 AND  pa.review_status = 3 GROUP BY pa.cid";
...
function get_user_coupons_list($user_id = '', $is_use = false, $total = '', $cart_goods = false, $user = true, $cart_ru_id = -1, $act_type = 'user') {
    ...
    $cart_where = '';
    if($cart_ru_id > -1){
        $cart_where .= " AND c.ru_id IN($cart_ru_id) AND cu.is_use = 0";
    }
    ...
    if ($is_use && isset($total) && $cart_goods) {
        ...
        foreach($res as $key=>$row){
            ...
            $sql = "SELECT c.*, cu.uc_id FROM " . $GLOBALS['ecs']->table("coupons_user") . " AS cu " .
                    ...
                    " AND cu.order_id = 0 AND cu.user_id = '$user_id'" . $cart_where;
        ...
        if (!empty($user_id) && $user) {
            ...
            $where = " WHERE cu.user_id IN(" . $user_id . ") AND c.review_status = 3 GROUP BY c.cou_id";
        }
        ...
        $sql = " SELECT c.*, cu.* $select FROM " .
                $GLOBALS['ecs']->table('coupons_user') . " AS cu LEFT JOIN " . $GLOBALS['ecs']->table('coupons') . " AS c ON c.cou_id = cu.cou_id " .
                $leftjoin . $where . $cart_where;
