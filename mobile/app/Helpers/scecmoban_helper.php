...
function presale_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
	...
	if ($res === NULL) {
		...
		if ($data === false) {
			...
			$res = $GLOBALS['db']->getAll($sql);
			$sql = 'SELECT pa.cid, COUNT(*) AS goods_num ' . 'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('presale_activity') . ' AS pa ON pa.goods_id = g.goods_id ' . ' WHERE is_delete = 0 AND  pa.review_status = 3 ' . ' GROUP BY pa.cid';
			$res2 = $GLOBALS['db']->getAll($sql);
			$sql = 'SELECT pc.cid, COUNT(*) AS goods_num FROM ' . $GLOBALS['ecs']->table('presale_cat') . ' AS pc ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('presale_activity') . ' AS pa ON pc.cid = pa.cid ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = pa.goods_id ' . 'WHERE  pa.review_status = 3 GROUP BY pa.cid';
