...
// 重组商家订货单数组  按照优惠活动对订货单商品进行分类 -qin
function cart_by_favourable($merchant_goods)
{
    foreach ($merchant_goods as $key => $row) { // 第一层 遍历商家
        ...
        foreach ($user_cart_goods as $key1 => $row1) { // 第二层 遍历订货单中商家的商品
            ...
            if (isset($sort_favourable['by_all']) && $row1['extension_code'] != 'package_buy') {
                foreach ($sort_favourable['by_all'] as $key2 => $row2) {
                    if ($row1['is_gift'] == 0) { // 活动商品
                        ...
                        $merchant_goods[$key]['new_list'][$row2['act_id']]['available'] = favourable_available($row2);// 订货单满足活动最低金额
                        // 订货单中已选活动赠品数量
 ...
 * 订货单中是否已经有某优惠
 * @param   array $favourable 优惠活动
 * @param   array $cart_favourable订货单中已有的优惠活动及数量
 */
