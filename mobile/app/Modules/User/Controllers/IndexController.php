...
class IndexController extends FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        $type = 0;
        $groupbuy_where = " and  oi.extension_code = 'group_buy'";
        $groupbuy_count = get_order_where_count($user_id, $type, $groupbuy_where);
        $this->assign('groupbuy_count', intval($groupbuy_count));//待付款

        $presale_where = " and  oi.extension_code = 'presale'";
        $presale_count = get_order_where_count($user_id, $type, $presale_where);
        $this->assign('presale_count', intval($presale_count));//待付款

        $sample_where = " and  oi.extension_code = 'sample'";
        $sample_count = get_order_where_count($user_id, $type, $sample_where);
        $this->assign('sample_count', intval($sample_count));//待付款

        $wholesale_where = " and  oi.extension_code = 'wholesale'";
        $wholesale_count = get_order_where_count($user_id, $type, $wholesale_where);
        $this->assign('wholesale_count', intval($wholesale_count));//待付款
        ...
        // 待发货
        $where_splited = " AND oi.pay_status = " . PS_PAYED . " AND oi.order_status in (" . OS_CONFIRMED . ", " . OS_SPLITED . ", " . OS_SPLITING_PART . ") AND (oi.shipping_status in (" . SS_UNSHIPPED .  ',' . SS_PREPARING.  ',' . SS_SHIPPED_ING ."))";
        $splite_count = get_order_where_count($user_id, $type, $where_splited);
        $this->assign('splite_count', intval($splite_count));//待收货

        //待收货
        $where_confirmed = " AND oi.pay_status = " . PS_PAYED . " AND oi.order_status in (" . OS_CONFIRMED . ", " . OS_SPLITED . ", " . OS_SPLITING_PART . ") AND (oi.shipping_status  in (" . SS_SHIPPED . "))";
        $confirmed_count = get_order_where_count($user_id, $type, $where_confirmed);
        $this->assign('confirmed_count', intval($confirmed_count));//待发货

        //已完成
        $where_completed =  " AND oi.order_status " . db_create_in(array(OS_CONFIRMED, OS_SPLITED)) . "  AND oi.shipping_status = '" . SS_RECEIVED . "' AND oi.pay_status " . db_create_in(array(PS_PAYED, PS_PAYING)) ;
        $completed_count = get_order_where_count($user_id, $type, $where_completed);
        $this->assign('completed_count', intval($completed_count));
