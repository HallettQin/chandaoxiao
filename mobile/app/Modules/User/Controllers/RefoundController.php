...
class RefoundController extends FrontendController
{
    ...
    public function actionApplyReturn()
    {
        ...
        /* 退货权限：订单状态 已发货、未退货 */
        $order_id = I('order_id', 0, 'intval');
        $order = dao('order_info')->field('order_id,shipping_time,pay_time, add_time, shipping_status, order_status, chargeoff_status, is_settlement, pay_status')->where(['order_id' => $order_id])->find();
        if (!$order || !$order['order_id']) {
            show_message( L('nonsupport_return_goods'), '', '', 'info' , true );
        }
                        ...
                        if ($value['cause'] == 3) {
                            if ($row['user_pay'] == PS_PAYED) {
                                if ($time > $order['pay_time'] + 1 * 3600 * 24) {
                                    //过了24小时不能退款
                                    unset($goods_cause_select[$key]);
                                } else {
                                    $goods_cause_select[$key]['is_checked'] = 1;
                                }
                            } else {
                                unset($goods_cause_select[$key]);
                            }
