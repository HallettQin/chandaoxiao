 ...
 * @return void
 */
function seller_order_list($page = 0)
{
    ...
    if ($result === false)
    {
        ...
        //by zxk 修复composite_status == "" bug
        $filter['composite_status'] = isset($_REQUEST['composite_status']) && $_REQUEST['composite_status'] != ""? intval($_REQUEST['composite_status']) : -1;

        //订单模式
        $extension_code = isset($_REQUEST['code']) ? trim($_REQUEST['code']) : "";
        if (in_array($extension_code, ['group_buy', 'presale', 'sample', 'wholesale'])) {
            $filter['extension_code'] = $extension_code;
        }
        ...
        if ($filter['extension_code']) {
            $where .= " AND o.extension_code = '$filter[extension_code]'";
        }
    ...
    foreach ($row AS $key => $value)
    {
        ...
        if ($value['chargeoff_status'] == 1 || $value['chargeoff_status'] == 2) {
            ...
            $bill = $GLOBALS['db']->getRow(" SELECT scb.id, scb.bill_sn, scb.seller_id, scb.proportion, commission_model FROM " . $GLOBALS['ecs']->table('seller_bill_order') . " AS sbo " .
                ...
                "WHERE sbo.order_id = '" . $value['order_id'] . "' AND return_rec_id = 0");
        ...
        $row[$key]['short_order_time'] = local_date('Y-m-d H:i:s', $value['add_time']);
        ...
        //款
        $row[$key]['order_goods_num'] = count($goods['goods_list']);

        $row[$key]['order_url'] = url('order/detail', ['order_id'=>$order['order_id']]);
        ...
        //添加订单状态
        $row[$key]['order_status'] = $os[$value[order_status]] . ',' . $ps[$value[pay_status]] . ',' . $ss[$value['shipping_status']];
    }
 ...
 * 格式 array('confirm' => true, 'pay' => true)
 */
function seller_operable_list($order)
{
    /* 取得订单状态、发货状态、付款状态 */
    $os = $order['order_status'];
    $ss = $order['shipping_status'];
...
function sys_msg($msg_detail, $msg_type = 0, $links = array(), $auto_redirect = true, $is_ajax = false) {
    show_message($msg_detail, $links[0]['text'], $links[0]['href']);
}
