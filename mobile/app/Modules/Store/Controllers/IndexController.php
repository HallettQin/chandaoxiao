...
class IndexController extends FrontendController
{
    ...
    public function actionShopInfo()
    {
        ...
        if (!$_SESSION['seller_id'] || $_SESSION['user_id'] != $userid) {
            //检查权限
            $this->purchasers_priv();
        }
        ...
        $view = dao('touch_page_view')->field('type, title, thumb_pic, data, default')->where(['ru_id' => $data['user_id']])->find();
        $list = $view['data'];
        ...
        $share_data = [
            ...
            'link' => '',
            'img' => get_wechat_image_path(ltrim($data['logo_thumb'], '../')), // 获得当前展厅logo缩略图
        ];
    ...
    protected function initParam() {
        ...
        if (!$mode) {
            $groupbuy_list = activity_list($cat_id, 'groupbuy', $this->merchant_id, 'act_id', 'DESC', '1', 1000);
            $sample_list = activity_list($cat_id, 'sample', $this->merchant_id, 'act_id', 'DESC', '1', 1000);
            $presale_list = activity_list($cat_id, 'presale', $this->merchant_id, 'act_id', 'DESC', '1', 1000);
            $wholesal_list = activity_list($cat_id, 'wholesale', $this->merchant_id, 'act_id', 'DESC', '1', 1000);
            $goodslist = array_merge($groupbuy_list['list'], $sample_list['list'], $presale_list['list'], $wholesal_list['list']);
        } else {
            $activity_list = activity_list($cat_id, $mode, $this->merchant_id, 'act_id', 'DESC', '1', 1000);
            $goodslist = $activity_list['list'];
        }

        //排序
        $goods_ids = array_column($goodslist, 'goods_id');
        array_multisort($goods_ids,SORT_NUMERIC, SORT_DESC, $goodslist);
    ...
    public function actionProList()
    {
        ...
        if (IS_AJAX) {
            ...
            $groupbuy_list = activity_list($cat_id, 'groupbuy', $ru_id, 'act_id', 'DESC', '1', 1000, $keyword);
            $sample_list = activity_list($cat_id, 'sample', $ru_id, 'act_id', 'DESC', '1', 1000, $keyword);
            $presale_list = activity_list($cat_id, 'presale' ,$ru_id, 'act_id', 'DESC', '1', 1000, $keyword);
            $wholesal_list = activity_list($cat_id, 'wholesale', $ru_id, 'act_id', 'DESC', '1', 1000, $keyword);
            $goodslist = array_merge($groupbuy_list['list'], $sample_list['list'], $presale_list['list'], $wholesal_list['list']);
