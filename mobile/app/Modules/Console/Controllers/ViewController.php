...
class ViewController extends FrontendController
{
            ...
            if ($goods_id) {
                ...
                foreach ($goods_id as $key => $val) {
                    ...
                    if($row){
                        ...
                        $goods[$key]['goods_id'] = price_format($row['shop_price']);
                        $goods[$key]['img'] = get_wechat_image_path($row['goods_thumb']);
                        $goods[$key]['goods_id'] = get_wechat_image_path($row['goods_img']);
    ...
    public function actionPurchasers() {
        if (isset($_SESSION['is_purchasers']) && $_SESSION['is_purchasers']) {
            exit(json_encode(['is_purchasers'=>1]));
        }
        exit(json_encode($_SESSION));
    }

    public function actionSuppliers() {
        if (isset($_SESSION['is_suppliers']) && $_SESSION['is_suppliers']) {
            exit(json_encode(['is_suppliers'=>1]));
        }
        exit(json_encode($_SESSION));
    }
}
