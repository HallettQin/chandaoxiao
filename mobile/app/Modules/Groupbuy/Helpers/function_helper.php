...
function get_merchant_group_goods($group_buy_id)
{
    ...
    $sql = "SELECT ga.act_id, ga.ext_info, ga.act_name, g.goods_thumb, g.sales_volume FROM ( SELECT * FROM " . $GLOBALS['ecs']->table('goods_activity') . " order by act_id desc)  ga"
        . " LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " g ON ga.goods_id = g.goods_id "
        . " WHERE ga.user_id = '$ru_id' AND act_type = '" . GAT_GROUP_BUY . "' AND ga.review_status = 3  order by ga.act_id desc LIMIT 4 ";
