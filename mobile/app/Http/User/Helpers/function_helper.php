...
function zc_focus_list($user_id = 0, $type = 1)
{
	...
	case 2:
		$where = ' AND ' . $now . ' > zp.start_time and ' . $now . ' < zp.end_time and zp.amount > zp.join_money';
		break;

	case 3:
		$where = ' AND ' . $now . ' > zp.start_time and zp.join_money >= zp.amount';
		break;
	...
	foreach ($zc_focus_list as $k => $z_val) {
		...
		$time = gmtime();
		if (($z_val['start_time'] < $time) && ($time < $z_val['end_time']) && ($z_val['join_money'] < $z_val['amount'])) {
			$zc_focus_list[$k]['zc_status'] = '活动进行中';
		}
		else {
			if (($z_val['start_time'] < $time) && ($z_val['amount'] <= $z_val['join_money'])) {
...
function zc_get_user_orders($user_id, $size = 10, $page = 1, $status = 0)
{
	...
	foreach ($res as $key => $row) {
		...
		$time = gmtime();
		if (($row['start_time'] < $time) && ($time < $row['end_time']) && ($row['join_money'] < $row['amount'])) {
			$zc_status = '活动进行中';
		}
		else {
			if (($row['start_time'] < $time) && ($row['amount'] <= $row['join_money'])) {
				$zc_status = '活动结束';
			}
			else {
				$zc_status = '活动失败';
...
function get_all_return_order($order_id = 0)
{
	...
	$sql = 'SELECT o.order_id, o.order_sn, o.add_time, o.extension_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o' . ' WHERE o.user_id = \'' . $_SESSION['user_id'] . '\' AND o.order_status  IN (\'1\',\'5\', \'6\')  AND o.pay_status  IN (\'2\',\'3\') ' . ' AND o.is_delete = 0 ' . ' AND o.pay_time > ' . $log_time . ' AND o.pay_time < ' . $time . $where . ' group by o.order_id ORDER BY o.add_time DESC';
