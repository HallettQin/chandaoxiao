...
class ApiSeller extends \App\Http\Admin\Controllers\EditorSeller
{
			...
			if (IS_POST) {
				...
				if ($id) {
					...
					if (count($num) == 1) {
						...
						$keep = array('ru_id' => $id, 'type' => 'store', 'title' => '展厅首页', 'data' => !empty($_POST['data']) ? $_POST['data'] : '', 'pic' => $pic, 'update_at' => $time, 'default' => 1);
	...
	public function actionMakeGallery()
	{
		if (IS_POST) {
			...
			$time = gmtime();
			$store = $ru_id + 100;
			$keep = array('album_id' => $store, 'ru_id' => $ru_id, 'album_mame' => '商家展厅相册', 'sort_order' => 50, 'add_time' => $time);
