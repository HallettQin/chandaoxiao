...
class Integrate
{
    ...
    public function set_session($username = '')
    {
        if (empty($username)) {
            ...
            $sql = "SELECT user_id, password, email, is_purchasers FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_name='$username' LIMIT 1";
            $row = $GLOBALS['db']->getRow($sql);
            ...
            if ($row) {
                ...
                $_SESSION['is_purchasers'] = $row['is_purchasers'];
                if (!$row['is_purchasers']) {
                    //不是采购商 查询是否是制造商
                    $sql = "SELECT user_id, ru_id, user_name, password, last_login, action_list, last_login,suppliers_id,ec_salt".
                        " FROM " . $GLOBALS['ecs']->table('admin_user') .
                        " WHERE ru_id = ". $_SESSION['user_id'];
                    $row2 = $GLOBALS['db']->getRow($sql);
                    if ($row2) {
                        //查找到
                        $_SESSION['is_suppliers'] = 1;
                        $_SESSION['admin_id'] = $row2['user_id'];
                        $_SESSION['seller_id'] = $row2['user_id'];
                        $_SESSION['admin_name'] = $row2['user_name'];
                        $_SESSION['seller_action_list'] = $row2['action_list'];
                    }
                }

                //注册时状态
                $sql = 'SELECT content ' . 'FROM ' .  $GLOBALS['ecs']->table('reg_extend_info') . ' WHERE user_id = ' . $row['user_id'] ." AND reg_field_id = 7";
