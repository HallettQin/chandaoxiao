...
/* JS代码增加_start  By bbs.siliang.com */
...
goods.too = false;
		// 判断商品详情页面，加进货单时，商品是否选择规格
		if(extCode && extCode!='yang_buy'){
			var b = 0;
			var c = 0;
			var t =gu_data.length;
			var t_m = 0;
			mun = parseInt($(".qingdan_info1").html());
			if(parseInt($("input[name='pifa_number']").val())>mun){
				$(".qidinl").show();
			}else{
				for(var e in gu_data){
					c =gu_data[e].array.length+c;
					t_m++;
					for(var k in gu_data[e].array){
						b++;
						spec_arr[0]=gu_data[e].id;
						spec_arr[1]=gu_data[e].array[k].id;

						goods.quick = quick;
						goods.extCode = extCode;
						goods.spec = spec_arr;
						goods.goods_id = goodsId;
						goods.number = gu_data[e].array[k].num;
						...
						Ajax.call('flow.php?step=add_to_cart', 'goods=' + JSON.stringify(goods), '', 'POST', 'JSON');
					}
					}
				}
...
/* 组合购买__添加商品到进货单__Start By bbs.siliang.com */
...
/* 组合购买__添加商品到进货单__End By bbs.siliang.com */
...
function MoveBox(gid) {
	var obj1 = $('#li_' + gid);
	if (obj1.length > 0) {
		flyCollect(gid, 'collectBox');// 飞入进货单
	} else {
		// 进货单页面加入操作，刷新页面
...
/* 代码增加_end By bbs.siliang.com */

/* 代码修改_start 整个替换掉即可 By bbs.siliang.com */
/*******************************************************************************
 * 评论的翻页函数
 */
 ...
 * 添加礼包到进货单
 */
/* 代码修改_start By bbs.siliang.com 这里增加了一个变量 */
...
function opencartDiv(price, name, pic, goods_brief, goods_id, total, number) {
	...
	// 生成层内内容
	html = '<div class=cardivfloat><span class=cartdivfloattitle>商品已成功添加到进货单！</span><a href=\'javascript:cancel_div()\' style="float:right;padding:0 26px 0 0;background:url(themes/68ecshopcom_360buy/images/ico_closebig1.gif) right center no-repeat;cursor:pointer;color:#ffffff;font-size:12px;" >关闭</a></div><div class="cartpopDiv"><div class="toptitle"><a href="goods.php?id=' + goods_id + '" class="pic"><img src=' + pic + ' width="98" height="98" style="border:#dddddd 1px solid; display:block;"/></a><p><font style="font-weight:bold">' + name + '</font>  <br>' + goods_brief + '<br>购买价格：<font style="color:#cc0000">' + price + '</font><br></p></div>';

	html += '<div class="coninfo">';
	html += '<table cellpadding="0" height="30"><tr><td align="center" >进货单共有<font style="color:#ff6701;"><strong>' + number + '</strong></font>种商品  合计：<font style="color:#cc0000;"><strong>' + total + '</strong></font></td></tr>';
	html += '</table>';
	html += '</div>';
