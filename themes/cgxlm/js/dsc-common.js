...
$(function(){
	...
	$("*[ectype='cateItem']").on('mouseenter',function(){
		...
		if(eveval != 1){
			T.data('eveval', '1');
			/*加载中by wu*/
			layer.find("*[ectype='subitems_" + cat_id + "']").html('<img src="themes/cgxlm/images/load/loadGoods.gif" width="200" height="200" class="lazy">');
			$.ajax({
			   type: "GET",
			   url: "ajax_dialog.php",
	...
	function logistics(){
		...
		$(document).on("click",".logistics_li",function(){
            ...
            var logistics_lis = $('.logistics_li');
            logistics_lis.each(function(index, value) {
                var $value = $(value);
                if ($value.data('ruid') == ru_id && $value.data('shipping') == shipping_id) {
                    $value.addClass("item-selected").siblings().removeClass("item-selected");

                    if (_html != "") {
                        $value.parents("*[ectype='disInfo']").find("*[ectype='tabLog'] span").html(_html);
                        $value.parents("*[ectype='disInfo']").find("*[ectype='tabLog']").attr("data-shipping",shipping_id).attr("data-shippingcode", shipping_code).attr("data-ruid", ru_id).attr("data-type", type);
                    }
                }
            })
            //t.addClass("item-selected").siblings().removeClass("item-selected");
	...
	function commentForm(obj){
        ...
        img_length = obj.find(".img-list-ul").length;
...
function openWin(obj) {
	$('#chat_2z2x_holder').trigger('click');
	return false;
    //判断是否登陆
    var user_id = $('input[name="user_id"]').val();
    if (user_id > 0) {
